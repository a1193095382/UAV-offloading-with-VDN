...
#网络参数
fre_carrier=3.45e9
MEMORY_CAPACITY=max_step*max_episode
EPSILON=0.9
...
class VDN:
    ...
    def learn(self,):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        b_memory_ = np.empty((BATCH_SIZE, 3, N_STATES * 2 + N_obs * 2 + 3))
        ...
        q_evals=q_evals.data.cpu().numpy()  #Q值相加
        q_targets=q_targets.data.cpu().numpy()
        q_evals=np.sum(q_evals,axis=2)
        q_targets = np.sum(q_targets, axis=2)
        q_total_eval = torch.FloatTensor(q_evals).to(device)
        q_total_eval.requires_grad_(True)
        q_total_target = torch.FloatTensor(q_targets).to(device)
        q_total_target .requires_grad_(True)
...
for episode in range(max_episode):
    ...
    for step in range(max_step):
        ...
        for j in range(n_agents):
            ...
            obs[j,:]=obs_[j,:]
            o=obs[j,:]
            agent_action[j],eval_hstate[j,:]=vdn.choose_action(o,last_a[j],eval_hstate[j,:],j)
        ...
        vdn.store_transition(exp)

        while vdn.memory_counter >1000:
